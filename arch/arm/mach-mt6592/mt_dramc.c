...
extern int g_dram_rank_count;
extern unsigned long g_1st_rank_saddr;
extern unsigned long g_1st_rank_size;
#endif
...
int DFS_APDMA_early_init(void)
{
   #ifdef APDMA_TEST
#if defined(CONFIG_MACH_LGE)
   if(g_dram_rank_count == 2) {
      src_array_p=(g_1st_rank_saddr+g_1st_rank_size)-IOREMAP_ALIGMENT;
      dst_array_p=(g_1st_rank_saddr+g_1st_rank_size);
   }
   else if(g_dram_rank_count == 1) {
#if 0
      src_array_p=0xBFD00000-IOREMAP_ALIGMENT;
      dst_array_p=0xBFD00000;
#else
      src_array_p=DRAM_BASE+(roundup(g_1st_rank_size,0x10000000))/2-IOREMAP_ALIGMENT;
      dst_array_p=DRAM_BASE+(roundup(g_1st_rank_size,0x10000000))/2;
#endif
   }
   else {
...
#if defined(CONFIG_MACH_LGE)
   if(g_dram_rank_count == 2) {
      src_array_p=(g_1st_rank_saddr+g_1st_rank_size)-BUFF_LEN/2;
      dst_array_p=(g_1st_rank_saddr+g_1st_rank_size)+BUFF_LEN/2;
   }
   else if(g_dram_rank_count == 1) {
#if 0
      src_array_p=0xBFD00000-BUFF_LEN/2;
      dst_array_p=0xBFD00000+BUFF_LEN/2;
#else
      src_array_p=DRAM_BASE+(roundup(g_1st_rank_size,0x10000000))/2-BUFF_LEN/2;
      dst_array_p=DRAM_BASE+(roundup(g_1st_rank_size,0x10000000))/2+BUFF_LEN/2;
#endif
   }
   else {
...
#if defined(CONFIG_MACH_LGE)
  if(g_dram_rank_count == 2) {
    DFS_dummy_read_center_address=g_1st_rank_saddr+g_1st_rank_size;
  }
  else if(g_dram_rank_count == 1) {
#if 0
    DFS_dummy_read_center_address=0xBFD00000;
#else
    DFS_dummy_read_center_address=DRAM_BASE+(roundup(g_1st_rank_size,0x10000000))/2;
#endif
  }
  else {
