...
void oam_run(void)
{
	...
	d5_count_time = 60;
#endif
#ifdef CONFIG_LGE_PM_OCV_UPDATE
...
#ifdef CONFIG_LGE_PM_OCV_UPDATE
#if defined(SOC_BY_SW_FG)
static int battery_meter_ocv_update(kal_int32 hw_ocv_after_sleep)
{
	static kal_int32 sleep_time = 0;

	kal_int32 ocv_diff = g_hw_ocv_before_sleep - hw_ocv_after_sleep;
	kal_int32 dod_by_ocv = fgauge_read_d_by_v(hw_ocv_after_sleep);

	kal_bool do_ocv_update = KAL_FALSE;
	kal_bool do_accumulate_car = KAL_FALSE;

	/* ocv update judgement */
	if (ocv_diff < 0) {
		/* ocv is increased. do nothing */
	} else if (ocv_diff < 5) {
		/* ocv is settled over 5 minute. update soc */
		if (_g_bat_sleep_total_time > LOW_POWER_WAKEUP_PERIOD) {
			do_ocv_update = KAL_TRUE;
		} else {
			do_accumulate_car = KAL_TRUE;
		}
	} else if (ocv_diff > 100) {
		/* ocv dropped too much. update soc */
		do_ocv_update = KAL_TRUE;
	} else {
		if (dod_by_ocv > oam_d_5)
			do_accumulate_car = KAL_TRUE;
	}
...
#if defined(SOC_BY_SW_FG)
	battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &hw_ocv_after_sleep);
	battery_meter_ocv_update(hw_ocv_after_sleep);
