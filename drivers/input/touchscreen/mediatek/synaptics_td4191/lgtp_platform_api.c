...
static int i2c_read(struct i2c_client *client, u8 *reg, int regLen, u8 *buf, int dataLen)
{
	...
	if( dataLen <= MAX_I2C_TRANSFER_SIZE )
	{
		...
		struct i2c_msg msgs[2] = {
			{ .addr = switch_i2c_addr, .flags = 0, .len = regLen, .buf = reg, },
			{ .addr = switch_i2c_addr, .flags = I2C_M_RD, .len = dataLen, .buf = buf, },
		};
...
static int i2c_write(struct i2c_client *client, u8 *reg, int regLen, u8 *buf, int dataLen)
{
    ...
    int retry = 0;
	struct i2c_msg msg = {
		.addr = switch_i2c_addr, .flags = 0, .len = (regLen+dataLen), .buf = NULL,
	};
	...
	#elif defined ( TOUCH_PLATFORM_MTK )
      for (retry = 0; retry < I2C_MAX_TRY; retry++) {
            if (i2c_msg_transfer(client, &msg, 1) < 0) {
                if (printk_ratelimit())
				    TOUCH_ERR("transfer error, "
						    "retry (%d)times\n", retry + 1);
			    msleep(20);
