...
static void lu201x_probe_regulators(struct i2c_client *client)
{
	...
	if (IS_ERR(vdd_io)) {
		...
		TOUCH_LOG("Error %d getting vdd_io regulator\n", error);
		goto fail;
	}

	error = regulator_set_voltage(vdd_io, 2950000, 2950000);
	if (error < 0) {
		TOUCH_LOG("Error %d cannot control vdd_io regulator\n", error);
		goto fail;
	}

	error = regulator_enable(vdd_io);
	if (error < 0) {
			TOUCH_LOG("vdd_io regulator enable fail\n");
			goto fail;
	}
	mdelay(1);
	TOUCH_LOG("%s VDD_IO value : %d\n", __func__, regulator_get_voltage(vdd_io));
...
static void WqfirmwareUpgrade(struct work_struct *work_upgrade)
{
	...
	if( pDriverData->useDefaultFirmware == TOUCH_FALSE || pDriverData->useDefaultFirmware == TOUCH_TRUE) {
...
static void touch_early_suspend(struct early_suspend *h)
{
	...
	if( pDriverData->nextState == STATE_NORMAL ) {
		TOUCH_ERR("Invalid next driver state ( state = STATE_NORMAL )\n");
		pDriverData->nextState = STATE_KNOCK_ON_ONLY; /* TBD : */
	}
...
static void touch_late_resume(struct early_suspend *h)
{
	...
	if( pDriverData->nextState != STATE_NORMAL ) {
		TOUCH_WARN("Invalid next driver state ( state = %d )\n", pDriverData->nextState);
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#if defined ( TOUCH_MODEL_Y30 )
	lu201x_probe_regulators(client);
	#endif
   ...
   // queue_delayed_work(touch_wq, &pDriverData->work_upgrade, 0);
	...
	//	if( pDriverData->binFwInfo.version != pDriverData->icFwInfo.version )
	    if( pDriverData->icFwInfo.version != 120)
		{
                TOUCH_ERR("binFwInfo.version:%d\n",pDriverData->binFwInfo.version);
                TOUCH_ERR("icFwInfo.version:%d\n",pDriverData->icFwInfo.version);
				pDriverData->useDefaultFirmware = TOUCH_TRUE;
				queue_delayed_work(touch_wq, &pDriverData->work_upgrade, 0);
		}
	}
...
static int __init touch_init(void)
{
	...
	if( TouchGetBootMode() == BOOT_OFF_CHARGING ) {
		TOUCH_LOG("Power Off Charging Detected, so do not probe touch driver.\n");
		TouchAssertReset(); /* to reduce current consumption */
		return -ENODEV;
	}
