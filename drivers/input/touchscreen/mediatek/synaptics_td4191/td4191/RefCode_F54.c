	...
	for (i = 0; i < (unsigned int)F12_2DTxCount; i++) {
		...
		for (j = 0; j < (unsigned int)F12_2DRxCount; j++) {
			if ((RT78Data[i][j] < RT78FullRawCapUpperLimit[i][j])
				...
				&&(RT78Data[i][j] > RT78ElectrodeShortLowerLimit[i][j])
				&&(RT78Data[i][j] < RT78ElectrodeShortUpperLimit[i][j])) {
				//TOUCH_INFO_MSG("TX[%d]RX[%d] passed value:  %d\n",
				//		i, j, RT78Data[i][j]);
				//result = true;
				outbuf += snprintf(f54_wlog_buf+outbuf, sizeof(f54_wlog_buf)-outbuf, "%5s ", " ,");//us10_porting
			} else {
				TOUCH_INFO_MSG("TX[%d]RX[%d] failed value:  %d\n",
						i, j, RT78Data[i][j]);
				outbuf += snprintf(f54_wlog_buf+outbuf, sizeof(f54_wlog_buf)-outbuf, "%5s ", "X,");//us10_porting
				result = false;
			}
		}
		outbuf += snprintf(f54_wlog_buf+outbuf, sizeof(f54_wlog_buf)-outbuf, "\n"); //us10_porting
...
int ReadRT78(char *buf)
{
	...
	for (i = 0; i < F12_2DTxCount; i++) {
		...
		for (j = 0; j < F12_2DRxCount; j++, k += 2) {
			...
			RT78Data[i][j] = temp;
			outbuf += snprintf(f54_wlog_buf+outbuf, sizeof(f54_wlog_buf)-outbuf, "%5d ", RT78Data[i][j]);//us10_porting
		}
		outbuf += snprintf(f54_wlog_buf+outbuf, sizeof(f54_wlog_buf)-outbuf, "\n"); //us10_porting
	}
	outbuf += snprintf(f54_wlog_buf+outbuf, sizeof(f54_wlog_buf)-outbuf,
...
int ImageTest(int mode, char *buf)
{
	...
	if (ret == 1) {
		...
		data = 78;//20;/*raw capacitance mode*/ //us10_porting
