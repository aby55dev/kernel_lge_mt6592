...
{
	if( eventIndex < NUM_OF_UEVENT && atomic_read(&uevent_state) == UEVENT_IDLE)
	{
		if(eventIndex == UEVENT_SWIPE && !lockscreen_stat ) {
			TOUCH_ERR("drop LPWG_SWIPE_DOWN event, lockscreen=%d\n", lockscreen_stat);
			return;
		}
		wake_lock_timeout(pWakeLockTouch, msecs_to_jiffies(3000));
		atomic_set(&uevent_state, UEVENT_BUSY);
...
static void WqTouchIrqHandler(struct work_struct *work_irq)
{
	...
	else if( readData.type == DATA_SWIPE )
	{
		if(lockscreen_stat) {
			...
			TOUCH_WARN("lockscreen_stat is %d, not report swipe \n",lockscreen_stat);
		}
	}
#endif
...
static ssize_t store_lpwg_notify(struct i2c_client *client, const char *buf, size_t count)
{
	...
	if( ( nextState != STATE_UNKNOWN ) && ( pDriverData->currState != nextState ) )
	{
		...
		#if defined ( TOUCH_DEVICE_S3320 )	 || defined ( TOUCH_DEVICE_MIT200 ) || defined ( TOUCH_DEVICE_TD4191 )
...
static ssize_t store_lpwg_data(struct i2c_client *client, const char *buf, size_t count)
{
...
#if defined ( TOUCH_MODEL_B2L ) || defined ( TOUCH_MODEL_L80 )  || defined ( TOUCH_MODEL_P1S3G )
	...
	atomic_set(&uevent_state, UEVENT_IDLE);
	wake_unlock(pWakeLockTouch);
	mutex_unlock(pMutexTouch);
...
static ssize_t show_version(struct i2c_client *client, char *buf)
{
	...
	ret += sprintf(buf+ret,	"IC_product_id[PLG496]\n");
	return ret;
}
...
static void touch_early_suspend(struct early_suspend *h)
{
	...
	#if defined ( TOUCH_DEVICE_S3320 ) || defined ( TOUCH_DEVICE_TD4191 )
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TOUCH_LOG("g_boot_mode: %d\n",g_boot_mode);
	/* Trigger firmware upgrade if needed */
	if( (pDriverData->binFwInfo.isOfficial || !pDriverData->binFwInfo.isOfficial) && g_boot_mode!=RECOVERY_BOOT)
	{
