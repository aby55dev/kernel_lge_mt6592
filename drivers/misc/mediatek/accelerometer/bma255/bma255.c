...
static int bma255_runCalibration(void)
{
	...
	if (res2 < 0)//change range before calibration
	{
		...
		if (res2 < 0)//change range before calibration
		{
			GSE_ERR("Second SetDataFormat 4G error");
			return FALSE;
		}
	}
...
#if defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
		if(err = bma255_create_attr(&(gsensor_init_info.platform_diver_addr->driver)))
		{
...
#if defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
	if(err = bma255_delete_attr(&(gsensor_init_info.platform_diver_addr->driver)))
	{
		GSE_ERR("bma255_delete_attr fail: %d\n", err);
	}
#else
...
{
	struct acc_hw *hw = bma255_get_cust_acc_hw();
...
static int __init bma255_init(void)
{
	...
	{
		GSE_ERR("HW REV_A : bma255 init FAIL\n");
		return -ENODEV;
	}
	#endif
