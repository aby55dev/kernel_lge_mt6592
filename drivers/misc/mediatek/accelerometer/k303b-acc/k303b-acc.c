...
static int k303b_CheckDeviceID(struct i2c_client *client)
{
	...
	{
        GSE_ERR("Check ID error: id = 0x%x != K303B_FIXED_DEVID[ 0x%x ]\n", databuf[0], K303B_FIXED_DEVID);
      ...
      if((res = hwmsen_read_block(client, 0x0 + i, databuf_reg, 1)))
	  	{
	        mdelay(5);
            if((res = hwmsen_read_block(client, 0x0 + i, databuf_reg, 1))){
            	GSE_LOG("k303b_StoreHiddenReg : Read Error Reg = 0X%x, data=%d\n", i, databuf_reg[0]);
            }
            else{
                GSE_LOG("k303b_StoreHiddenReg : Reg = 0X%x, data=%d\n",i, databuf_reg[0]);
                ReservedRegData[i]=databuf_reg[0];
            }
        }
      else
        {
            printk("[gsensor] k303b_StoreHiddenReg : Reg = 0X%x, data=%d\n",i, databuf_reg[0]);
            ReservedRegData[i]=databuf_reg[0];
        }
	  databuf_reg[0] = 0;
	  databuf_reg[1] = 0;
	  ...
	  if((res = hwmsen_read_block(client, 0x0 + i, databuf_reg, 1)))
		{
			mdelay(5);
			if((res = hwmsen_read_block(client, 0x0 + i, databuf_reg, 1))){
				printk("[gsensor] k303b_StoreHiddenReg : Read Error Reg = 0X%x, data=%d\n", i, databuf_reg[0]);
			}
			else{
...
static int k303b_SetBWRate(struct i2c_client *client, u8 bwrate)
{
    ...
    **/
	//GSE_LOG("recv data ");
	//bwrate = ((ODR_ACC_MASK & bwrate) | ((~ODR_ACC_MASK) & databuf[0]) | K303B_ACC_ODR_ENABLE);
...
static int k303b_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	if(res <= 0)
	{
		GSE_ERR("k303b_reg_data_format Error! send Error!!\n");
		return K303B_ERR_I2C;
	}
	...
	if(res <= 0)
	{
		GSE_ERR("k303b_reg_data_format Error! recv Error!!\n");
		return K303B_ERR_I2C;
	}
	...
	if(res <= 0)
	{
		GSE_ERR("k303b_reg_data_format Error! send 2 Error!!\n");
		return K303B_ERR_I2C;
	}
...
static int k303b_init_client(struct i2c_client *client, int reset_cali)
{
    ...
    if(res != K303B_SUCCESS)
    {
        GSE_ERR("K303B_SetIntEnable error\n");
          return res;
    }
    ***/
	// 7 cali
...
//TIME_LOG_START();
	...
	//for Hidden Register issue
		...
		if(hiddenreg_read_count > 100)
		{
			...
			GSE_LOG("Check HiddenRegister Start\n");
			for(i=1;i<5;i++)
			{
				if((err = hwmsen_read_block(client, 0x0 + i, reserved, 1)))
				{
					...
					if((err = hwmsen_read_block(client, 0x0 + i, reserved, 1)))
					{
						GSE_LOG("Read Error for HiddenReg : 0x%x, data=%d\n",i,reserved[0]);
					}
					else
					{
			...
			if(!recovery_monitor_started) {
				...
				if(norm >= RECOVERY_NORM_THRESHOLD) {
					...
					if(recovery_monitor_counter > RECOVERY_MONITOR_COUNT_MAX) {
						GSE_LOG("[3] Call hiddenregister recovery function\n");
						k303b_HiddenReg_Recovery(client);
						recovery_monitor_started = 0;
						recovery_monitor_counter = 0;
						//GSE_LOG("RECOVERY RESERVED REGISTERS !!!!\n");
					}
				}
			}
		}
...
static int k303b_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if(sensor_power == FALSE)
	{
		...
		if(res)
		{
			GSE_ERR("Power on k303b error %d!\n", res);
		}
		msleep(20);////////////////////////////////////////////////????check
	}
	...
	else
	{
        ...
        //GSE_LOG("K303B_ReadSensorData %d %d %d", acc[K303B_AXIS_X], acc[K303B_AXIS_Y], acc[K303B_AXIS_Z]);
		data_count++;
	   ...
	   if((res = hwmsen_read_block(client, 0x0 + i, databuf_reg, 1)))
		 {
			 mdelay(5);
			 if((res = hwmsen_read_block(client, 0x0 + i, databuf_reg, 1))){
				 GSE_LOG("show_hidden_value : Read Error Reg = 0X%x, data=%d\n", i, databuf_reg[0]);
			 }
			 else{				 
...
static int k303b_do_calibration(void)
{
      ...
      /* Sensor stable time after power on  */
       msleep ( 300 ); 
        ...
        if(err < 0)
        {
            GSE_ERR("%s : k303b_ReadData() failed\n", __func__);
            return err;
        }
        ...
        for(i = 0; i < CALIBRATION_DATA_AMOUNT; i++)
        {
            ...
            if(err < 0)
            {
                GSE_ERR("%s : k303b_ReadData() failed in the %dth loop\n", __func__, i);
                return err;
            }
	...
	obj->cali_sw[K303B_AXIS_Y] = -(s16)(sum[K303B_AXIS_Y] / CALIBRATION_DATA_AMOUNT);	//K303B(16bit) 0+-154
        ...
        GSE_LOG("%s : calibration data (%d, %d, %d)\n", __func__, obj->cali_sw[K303B_AXIS_X], obj->cali_sw[K303B_AXIS_Y], obj->cali_sw[K303B_AXIS_Z]);
		GSE_LOG("%s: err= %d \n",__func__,err);
        return K303B_SUCCESS;
}
...
static ssize_t k303b_runCalibration_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
		GSE_LOG("self calibration K303B_ERR_SETUP_FAILURE\n");
...
static int k303b_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		GSE_ERR("k303b_device register failed\n");
...
static int k303b_i2c_remove(struct i2c_client *client)
{
	...
	{
		GSE_ERR("k303b_delete_attr fail: %d\n", err);
	}
#else
