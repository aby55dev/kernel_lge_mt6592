...
//[LGE_UPDATE_S][yonghwan.lym@lge.com][2015-04-01] add Interface(sensor information) for ACUTE
UINT32 OV5670MIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate)
{
    ...
    switch (scenarioId) {
        ...
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
             *pframeRate = imgsensor_info.pre.max_framerate;
             break;
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        case MSDK_SCENARIO_ID_CAMERA_ZSD:
             *pframeRate = imgsensor_info.cap.max_framerate;
            break;
...
kal_uint32 GetOv5670PixelClock(void)
{
    kal_uint32 PixelClock = imgsensor_info.pre.pclk;

    if(imgsensor.sensor_mode == IMGSENSOR_MODE_PREVIEW)
        PixelClock = imgsensor_info.pre.pclk;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE)
        PixelClock = imgsensor_info.cap.pclk;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_VIDEO)
        PixelClock = imgsensor_info.normal_video.pclk;
...
{
    kal_uint32 LineLength = imgsensor_info.pre.linelength;

    if(imgsensor.sensor_mode == IMGSENSOR_MODE_PREVIEW)
        LineLength = imgsensor_info.pre.linelength;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE)
        LineLength = imgsensor_info.cap.linelength;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_VIDEO)
        LineLength = imgsensor_info.normal_video.linelength;
...
{
    kal_uint32 FrameLength = imgsensor_info.pre.framelength;

    if(imgsensor.sensor_mode == IMGSENSOR_MODE_PREVIEW)
        FrameLength = imgsensor_info.pre.framelength;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE)
        FrameLength = imgsensor_info.cap.framelength;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_VIDEO)
        FrameLength = imgsensor_info.normal_video.framelength;

    return FrameLength;
}


kal_uint32 GetOv5670LinePixel(void)
{
    kal_uint32 LinePixel = imgsensor_info.pre.grabwindow_width;

    if(imgsensor.sensor_mode == IMGSENSOR_MODE_PREVIEW)
        LinePixel = imgsensor_info.pre.grabwindow_width;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE)
        LinePixel = imgsensor_info.cap.grabwindow_width;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_VIDEO)
        LinePixel = imgsensor_info.normal_video.grabwindow_width;

    return LinePixel;
}

kal_uint32 GetOv5670ActiveHeight(void)
{
    kal_uint32 ActiveHeight = imgsensor_info.pre.grabwindow_height;

    if(imgsensor.sensor_mode == IMGSENSOR_MODE_PREVIEW)
        ActiveHeight = imgsensor_info.pre.grabwindow_height;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE)
        ActiveHeight = imgsensor_info.cap.grabwindow_height;
    else if(imgsensor.sensor_mode == IMGSENSOR_MODE_VIDEO)
        ActiveHeight = imgsensor_info.normal_video.grabwindow_height;

    return ActiveHeight;
}
//[LGE_UPDATE_E][yonghwan.lym@lge.com][2015-04-01] add Interface(sensor information) for ACUTE
...
static kal_uint32 open(void)
{
    ...
    imgsensor.mirror = IMAGE_NORMAL; //LGE_UPDATE [youmi.jun@lge.com] 2015/02/12, modify sub camera orientation (OV5670 cowell module)
    spin_unlock(&imgsensor_drv_lock);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    SENSOR_INFO_FOR_ACUTE *pSensorInfoDataForAcute = (SENSOR_INFO_FOR_ACUTE*) feature_para; //[LGE_UPDATE][yonghwan.lym@lge.com][2015-04-01] add Interface(sensor information) for ACUTE
    ...
    switch (feature_id) {
        ...
        //[LGE_UPDATE_S][yonghwan.lym@lge.com][2015-04-01] add Interface(sensor information) for ACUTE
        case SENSOR_FEATURE_GET_SENSOR_INFO_FOR_ACUTE:
            {
                LOG_INF("SENSOR_FEATURE_GET_SENSOR_INFO_FOR_ACUTE\n");
                OV5670MIPIGetDefaultFramerateByScenario(imgsensor.current_scenario_id,&(pSensorInfoDataForAcute->max_fps));
                pSensorInfoDataForAcute->max_fps = pSensorInfoDataForAcute->max_fps/10; //300 -> 30
                pSensorInfoDataForAcute->pclk = GetOv5670PixelClock();
                pSensorInfoDataForAcute->line_length = GetOv5670LineLength();
                pSensorInfoDataForAcute->frame_length = GetOv5670FrameLength();
                pSensorInfoDataForAcute->line_pixel = GetOv5670LinePixel();
                pSensorInfoDataForAcute->active_height = GetOv5670ActiveHeight();
                *feature_para_len=sizeof(SENSOR_INFO_FOR_ACUTE);
                break;
            }
        //[LGE_UPDATE_E][yonghwan.lym@lge.com][2015-04-01] add Interface(sensor information) for ACUTE
        default:
