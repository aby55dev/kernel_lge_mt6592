...
#define OV8858_DEBUG
#ifdef OV8858_DEBUG
    #define OV8858DB(fmt, arg...)    xlog_printk(ANDROID_LOG_DEBUG , "[OV8858]", fmt, ##arg)
#else
...
kal_uint32 GetOv8858PixelClock(void)
{
    kal_uint32 PixelClock = OV8858_PREVIEW_PCLK;

    if(ov8858.sensorMode == SENSOR_MODE_PREVIEW)
        PixelClock = OV8858_PREVIEW_PCLK;
    else if(ov8858.sensorMode == SENSOR_MODE_CAPTURE)
        PixelClock = OV8858_CAPTURE_PCLK;
    else if(ov8858.sensorMode == SENSOR_MODE_VIDEO)
        PixelClock = OV8858_VIDEO_PCLK;

    return PixelClock;
}

kal_uint32 GetOv8858LinePixel(void)
{
    kal_uint32 LinePixel = OV8858_IMAGE_SENSOR_PV_WIDTH;

    if(ov8858.sensorMode == SENSOR_MODE_PREVIEW)
        LinePixel = OV8858_IMAGE_SENSOR_PV_WIDTH;
    else if(ov8858.sensorMode == SENSOR_MODE_CAPTURE)
        LinePixel = OV8858_IMAGE_SENSOR_FULL_WIDTH;
    else if(ov8858.sensorMode == SENSOR_MODE_VIDEO)
        LinePixel = OV8858_IMAGE_SENSOR_VIDEO_WIDTH;

    return LinePixel;
}

kal_uint32 GetOv8858ActiveHeight(void)
{
    kal_uint32 ActiveHeight = OV8858_IMAGE_SENSOR_PV_HEIGHT;

    if(ov8858.sensorMode == SENSOR_MODE_PREVIEW)
        ActiveHeight = OV8858_IMAGE_SENSOR_PV_HEIGHT;
    else if(ov8858.sensorMode == SENSOR_MODE_CAPTURE)
        ActiveHeight = OV8858_IMAGE_SENSOR_FULL_HEIGHT;
    else if(ov8858.sensorMode == SENSOR_MODE_VIDEO)
        ActiveHeight = OV8858_IMAGE_SENSOR_VIDEO_HEIGHT;
...
extern int g_lge_camera_flag; //13MP:3, 8MP:2 //UPDATE_LGE [youmi.jun@lge.com] 2015/03/24, apply camera SBP
...
// [LGE_UPDATE_E][yonghwan.lym@lge.com][2015-01-29] LGIT and Sunny module check for Y90
    ...
    //LGE_UPDATE_S [youmi.jun@lge.com] 2015/03/24, apply camera SBP
    if (g_lge_camera_flag == 3 /*13MP*/)
    {
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
    //LGE_UPDATE_E [youmi.jun@lge.com] 2015/03/24, apply camera SBP
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    SENSOR_INFO_FOR_ACUTE *pSensorInfoDataForAcute = (SENSOR_INFO_FOR_ACUTE*) pFeaturePara; //[LGE_UPDATE][yonghwan.lym@lge.com][2015-04-01] add Interface(sensor information) for ACUTE
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_SENSOR_INFO_FOR_ACUTE:
            {
                OV8858DB("SENSOR_FEATURE_GET_SENSOR_INFO_FOR_ACUTE\n");
                OV8858MIPIGetDefaultFramerateByScenario(OV8858CurrentScenarioId,&(pSensorInfoDataForAcute->max_fps));
                pSensorInfoDataForAcute->max_fps = pSensorInfoDataForAcute->max_fps/10; //300 -> 30
                pSensorInfoDataForAcute->pclk = GetOv8858PixelClock();
                pSensorInfoDataForAcute->line_length = GetOv8858LineLength();
                pSensorInfoDataForAcute->frame_length = GetOv8858FrameLength();
                pSensorInfoDataForAcute->line_pixel = GetOv8858LinePixel();
                pSensorInfoDataForAcute->active_height = GetOv8858ActiveHeight();
                *pFeatureParaLen=sizeof(SENSOR_INFO_FOR_ACUTE);
                break;
            }
        //[LGE_UPDATE_E][yonghwan.lym@lge.com][2015-04-01] add Interface(sensor information) for ACUTE
        default:
