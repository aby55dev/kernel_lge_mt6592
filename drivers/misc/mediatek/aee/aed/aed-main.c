...
int DumpThreadNativeInfo(struct aee_oops *oops)
{
	...
	ret = copy_from_user((void *)(oops->userthread_stack.Userthread_Stack), (const void __user *)(userstack_start), length);
	{
		int i=0;
		for (i=0;i<20;i++)
				LOGD("%x\n", oops->userthread_stack.Userthread_Stack[i]);
	}	
	...
	{
		LOGE(" K64+ U64 pc/lr/sp 0x%16lx/0x%16lx/0x%16lx\n", (long)(user_ret->user_regs.regs[32]), (long)(user_ret->user_regs.regs[30]),
				 (long)(user_ret->user_regs.regs[31]) );
			userstack_start = (unsigned long)user_ret->user_regs.regs[31];
		vma = current_task->mm->mmap;
		while (vma != NULL) 
		{
			if (vma->vm_start <= userstack_start && vma->vm_end >= userstack_start) {
			     ...
			     const char *msg)
{
	...
	if (NULL != oops) {
		...
		if(db_opt & DB_OPT_NATIVE_BACKTRACE)
		{
			...
			{
				LOGE("%s: oops->userthread_stack.Userthread_Stack kmalloc fail", __func__);
				return;
			}
			LOGE("%s: oops->userthread_stack.Userthread_Stack kmalloc:0x%08lx", __func__,(long)oops->userthread_stack.Userthread_Stack);
