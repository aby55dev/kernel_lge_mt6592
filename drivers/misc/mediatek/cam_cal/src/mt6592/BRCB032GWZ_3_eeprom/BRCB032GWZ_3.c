...
//   CAM_CALDB("[S24EEPORM] iReadData \n" );
   ...
   if (ui4_offset + ui4_length >= 0x2000)
   {
      CAM_CALDB("[BRCB032GWZ_3_eeprom] Read Error!! S-BRCB032GWZ_3_eeprom not supprt address >= 0x2000!! \n" );
      return -1;
   }
...
#endif //COMMON_CAM_CAL_DRV
{
    ...
    else
    {
        ...
        if(NULL == pBuff)
        {
            CAM_CALDB("[BRCB032GWZ_3_eeprom] ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                CAM_CALDB("[BRCB032GWZ_3_eeprom] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    if(NULL == pWorkingBuff)
    {
     ...
     CAM_CALDB("[BRCB032GWZ_3_eeprom] init Working buffer address 0x%8x  command is 0x%8x\n", (u32)pWorkingBuff, (u32)a_u4Command);
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:    
            CAM_CALDB("[BRCB032GWZ_3_eeprom] Write CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            CAM_CALDB("[BRCB032GWZ_3_eeprom] Read CMD \n");
#ifdef CAM_CALGETDLT_DEBUG            
            do_gettimeofday(&ktv1);
#endif 
            CAM_CALDB("[BRCB032GWZ_3_eeprom] offset %d \n", ptempbuf->u4Offset);
            CAM_CALDB("[BRCB032GWZ_3_eeprom] length %d \n", ptempbuf->u4Length);
            CAM_CALDB("[BRCB032GWZ_3_eeprom] Before read Working buffer address 0x%8x \n", (u32)pWorkingBuff);

            //i4RetValue = iReadData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
            CAM_CALDB("[BRCB032GWZ_3_eeprom] After read Working buffer data  0x%4x \n", *pWorkingBuff);

            //[LGE_UPDATE_S][yonghwan.lym@lge.com][2015-04-14] awb_cam_cal recovery patch
            if(((ptempbuf->u4Offset & 0xFFF00000) == 0xFAF00000))
            {
                EEPROM_Address[0] = 0; // address reset
                EEPROM_Address[1] = 0;
            }
            //[LGE_UPDATE_E][yonghwan.lym@lge.com][2015-04-14] awb_cam_cal recovery patch
            ...
            else if(ptempbuf->u4Offset == 0xFFFFFFFF)
            {
                ...
                CAM_CALDB("[BRCB032GWZ_3_eeprom] Shading CheckSum MSB=> %x %x\n",pWorkingBuff[0], pWorkingBuff[1]);
            }
            else
            {
            	//i4RetValue = iReadData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
               ...
               {               	 
                   ...
                   if(0<test_retry)
                   {
                       CAM_CALDB("[BRCB032GWZ_3_eeprom] Test error (%d) Read retry (%d) \n", test_retry,i4RetValue);
                       i4RetValue = -1;
                       test_retry--;
                   }
                   #endif
                   CAM_CALDB("[BRCB032GWZ_3_eeprom] error (%d) Read retry (%d) \n", i4RetValue,readTryagain);
                   if(i4RetValue != 0)
                   {
                       readTryagain--;
                   }
            ...
            break;
        default :
      	    CAM_CALDB("[BRCB032GWZ_3_eeprom] No CMD \n");
            i4RetValue = -EPERM;
        break;
    }
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        CAM_CALDB("[BRCB032GWZ_3_eeprom] to user length %d \n", ptempbuf->u4Length);
        CAM_CALDB("[BRCB032GWZ_3_eeprom] to user  Working buffer address 0x%8x \n", (u32)pWorkingBuff);
...
{
    CAM_CALDB("[BRCB032GWZ_3_eeprom] CAM_CAL_Open\n");
    spin_lock(&g_CAM_CALLock);
    if(g_u4Opened)
    {
        spin_unlock(&g_CAM_CALLock);
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        CAM_CALDB("[BRCB032GWZ_3_eeprom] Allocate device no failed\n");
    ...
    {
        CAM_CALDB("[BRCB032GWZ_3_eeprom] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        CAM_CALDB("[BRCB032GWZ_3_eeprom] Allocate mem for kobject failed\n");
    ...
    {
        CAM_CALDB("[BRCB032GWZ_3_eeprom] Attatch file operation failed\n");
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {             
int i4RetValue = 0;
    CAM_CALDB("[BRCB032GWZ_3_eeprom] Attach I2C \n");
//    spin_lock_init(&g_CAM_CALLock);
    ...
    CAM_CALDB("[BRCB032GWZ_3_eeprom] g_pstI2Cclient->addr = 0x%8x \n",g_pstI2Cclient->addr);
    //Register char driver
    i4RetValue = RegisterCAM_CALCharDrv();

    if(i4RetValue){
        CAM_CALDB("[BRCB032GWZ_3_eeprom] register char device failed!\n");
        return i4RetValue;
    }


    CAM_CALDB("[BRCB032GWZ_3_eeprom] Attached!! \n");
    return 0;                                                                                       
} 
...
static int __init CAM_CAL_i2C_init(void)
{
    ...
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALDB("failed to register BRCB032GWZ_3_eeprom driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALDB("failed to register BRCB032GWZ_3_eeprom driver, 2nd time\n");
        return -ENODEV;
    }	
