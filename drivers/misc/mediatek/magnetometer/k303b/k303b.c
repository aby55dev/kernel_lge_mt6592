...
static int k303b_CheckDeviceID(struct i2c_client *client)
{
	...
	{
        MSE_ERR("Check ID error: id = 0x%x != K303B_FIXED_DEVID[ 0x%x ]\n", databuf[0], K303B_FIXED_DEVID);
...
static int k303b_SetPowerMode(struct i2c_client *client, bool enable)
{
    ...
    MSE_LOG("k303b_SetPowerMode end!\n");
	sensor_power = enable;
	return 0;
}
...
/*
	if(sensor_power == FALSE)
	{
		...
		if(res)
		{
			MSE_ERR("Power on k303b error %d!\n", res);
		}
		msleep(20);
	}
*/
	...
	else
	{
		...
		**/
		...
		if((Count%500)==0)
        		MSE_LOG("limingggMSE %d %d %d", mag[K303B_AXIS_X], mag[K303B_AXIS_Y], mag[K303B_AXIS_Z]);
...
static ssize_t show_debug_value(struct device_driver *ddri, char *buf)
{
	...
	if(NULL == obj)
	{
		MSE_ERR("k303b_i2c_data is null!!\n");
		return 0;
	}
...
//	MSE_FUN(f);
	...
	err_out:
		//MSE_LOG("===========liangruo k303b ioctl done err = %d, =======\r\n", retval);
	return retval;
}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{
				value = *(int *)buff_in;
				MSE_LOG("simugyro enable value = %d\n",value);
				k303b_SetPowerMode(k303b_i2c_client, value);
				read_lock(&k303bmid_data.ctrllock);
				if(value == 1)
				{
				...
				wake_up(&open_wq);
				read_unlock(&k303bmid_data.ctrllock);
			MSE_LOG("#h#j# all:%x(%d) gyro:%x(%x)\n",get_sensors_status(LSM_ALL_SENSORS),value,get_sensors_status(SENSOR_GYROSCOPE),k303bmid_data.controldata[7]);
				// Do nothing
			}
			break;
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{
				value = *(int *)buff_in;
				MSE_LOG("rotation vector enable value = %d\n",value);
				k303b_SetPowerMode(k303b_i2c_client, value);
				read_lock(&k303bmid_data.ctrllock);
				if(value == 1)
				{
...
//#endif
		...
		{
			MSE_ERR("k303b_device register failed\n");
...
/*#ifdef K303B_M_NEW_ARCH
	...
	{
		MSE_ERR("k303b_delete_attr fail: %d\n", err);
	}
#else
...
static int  k303b_local_init(void)
{
   struct mag_hw *hw = get_cust_mag_hw();
	...
	{
	   MSE_ERR("k303b_local_init add driver error\n");
	   return -1;
	}
