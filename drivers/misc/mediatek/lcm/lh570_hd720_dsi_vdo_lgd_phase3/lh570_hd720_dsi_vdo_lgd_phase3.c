...
extern DSI_STATUS DSI_SetMode(unsigned int mode);
extern DPI_STATUS DPI_DisableClk(void);
extern DSI_STATUS DSI_Reset(void);
extern void DSI_MixMode_Start(void);
...
if(wakeup_by_swipe == false){
	...
	//if(!is_update)mutex_unlock(pMutexTouch);
#else
    MDELAY(1);
	DSI_clk_HS_mode(1);
	MDELAY(120);
...
//extern void chargepump_dsv_reset();
