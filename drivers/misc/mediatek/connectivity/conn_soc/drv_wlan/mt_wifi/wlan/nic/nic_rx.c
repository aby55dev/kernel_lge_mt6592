...
VOID nicRxProcessMonitorPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
...
#else
	ucRxMode = RX_VT_MIXED_MODE;
	rRadiotapFieldVendor.ucData = ucRxMode;
...
#if 0
	/* Bit Number 2 RATE */	
	if ((u4ItPresent & RADIOTAP_FIELD_RATE)) {
...
#else
	rMonitorRadiotap.ucRate = RATE_1M;
#endif
...
#else
	rMonitorRadiotap.ucAntennaSignal = 0;
	rMonitorRadiotap.ucAntennaNoise = 0;
	rMonitorRadiotap.ucAntenna = 0;

	rMonitorRadiotap.ucMcsKnown = ( BITS(0,6) |
									(((0 & BIT(1)) >> 1) << 7)
									);

	rMonitorRadiotap.ucMcsFlags = ( (0) |
									(0 << 2) |
									((ucRxMode & BIT(0)) << 3) |
									(0 << 4) |
									(0 << 5) |
	...
	/* Bit[6:0] for 802.11n, mcs0 ~ mcs7 */
	rMonitorRadiotap.ucMcsMcs = BIT(0);  
#endif
